...
в первых сборках ронял задание; рабочая последовательность восстановления найдена
прямым USB-зондом на живом принтере (`tools/capt-probe.py`, страница допечаталась без
выключения питания) и перенесена в `patches/lbp2900-paper-out-recovery.patch`; через
CUPS с этой сборкой ещё не проверялось.
...
- «Кончилась бумага». Что делает живой LBP2900 (зонд `tools/capt-probe.py`,
  ...
  страница. Если лоток всё ещё пуст: мигать индикатором (кнопка видна только при
  мигании), GoOffline, ждать кнопку, сброс ещё раз. Три прежние неудачные сборки:
  ...
  сценариев. Через CUPS с этой сборкой пока не проверено.
