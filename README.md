...
тоже работает на живом принтере (04.09.2026, через CUPS): драйвер повторяет
страницу, пока не появится бумага, индикатор мигает, кнопку нажимать не нужно.
...
3. Проверьте многостраничный документ и ситуацию «кончилась бумага». LBP2900 сам
   не продолжает: он сбрасывает недопечатанную страницу и раз в 15 секунд заново
   проверяет лоток. Драйвер это видит, каждый раз заново занимает принтер,
   сбрасывает его и посылает ту же страницу; индикатор мигает, в очереди macOS
   висит «нет бумаги». Достаточно положить бумагу: на следующем круге страница
   допечатается, кнопка не нужна. Счётчики страниц в логе (`pages a/b/c/d`)
   сквозные с момента включения принтера, а не по заданию; после
   переинициализации они обнуляются.
